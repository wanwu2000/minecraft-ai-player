package com.aiplayer.client.command;
...
public class CommandRegistry {
     ...
     * 注册 AI 相关命令
     */
    private void registerAISlashCommands(net.fabricmc.brigadier.CommandDispatcher<FabricClientCommandSource> dispatcher) {
        dispatcher.register(net.fabricmc.fabric.api.client.command.v2.ClientCommandManager.literal("ai")
            ...
            .then(net.fabricmc.fabric.api.client.command.v2.ClientCommandManager.literal("chat")
                .then(net.fabricmc.fabric.api.client.command.v2.ClientCommandManager.argument("message", StringArgumentType.string()).executes(context -> {
                    handleAIChat(context.getArgument("message", String.class), context);
                    return 1;
                }))
            )
            ...
            .then(net.fabricmc.fabric.api.client.command.v2.ClientCommandManager.literal("task")
                .then(net.fabricmc.fabric.api.client.command.v2.ClientCommandManager.argument("task", StringArgumentType.string()).executes(context -> {
     ...
     * 注册彗星相关命令
     */
    private void registerCometCommands(net.fabricmc.brigadier.CommandDispatcher<FabricClientCommandSource> dispatcher) {
        dispatcher.register(net.fabricmc.fabric.api.client.command.v2.ClientCommandManager.literal("mc")
            .then(net.fabricmc.fabric.api.client.command.v2.ClientCommandManager.literal("enable")
                .then(net.fabricmc.fabric.api.client.command.v2.ClientCommandManager.argument("module", StringArgumentType.string()).executes(context -> {
            ...
            .then(net.fabricmc.fabric.api.client.command.v2.ClientCommandManager.literal("list").executes(context -> {
                context.getSource().sendFeedback(Text.literal(cometBridge.listModules()), false);
                return 1;
            }))
     ...
     * 注册系统命令
     */
    private void registerSystemCommands(net.fabricmc.brigadier.CommandDispatcher<FabricClientCommandSource> dispatcher) {
        ...
        dispatcher.register(net.fabricmc.fabric.api.client.command.v2.ClientCommandManager.literal("exec")
            .then(net.fabricmc.fabric.api.client.command.v2.ClientCommandManager.argument("command", StringArgumentType.string()).executes(context -> {
    ...
    private void handleAIChat(String message, net.fabricmc.fabric.api.client.command.v2.ClientCommandContext<FabricClientCommandSource> context) {
        ...
        if (response != null) {
            context.getSource().sendFeedback(Text.literal("[AI] " + response), false);
        }
    }
    ...
    private void registerTaskParsers() {
        ...
        registerParser("highway", sentence -> 
            sentence.contains("高速") || sentence.contains("公路"),
            () -> {
                chatSystem.setCurrentTask("建造高速公路");
                if (cometBridge.isAvailable()) {
                    cometBridge.enableModule("scaffold");
